...
int main(int argc, char *argv[])
{
    ...
    if(verify == 1)
                sprintf(header,"customers,products,brands,function,IC,Cycles,CPI,CT,ET,L1_dcache_miss_rate,L1_dcache_misses,L1_dcache_accesses,correctness");
    else
        sprintf(header,"customers,products,brands,function,IC,Cycles,CPI,CT,ET,L1_dcache_miss_rate,L1_dcache_misses,L1_dcache_accesses");
            ...
            for(auto & p_size: p_sizes ) {
                for(auto & b_size: b_sizes ) {
                    ...
                    for(auto & function : functions) {
                        ...
                        sprintf(preamble, "%lu,%lu,%lu,%s,",c_size,p_size,b_size,function.c_str());
                        perfstats_init();
                        perfstats_enable();
